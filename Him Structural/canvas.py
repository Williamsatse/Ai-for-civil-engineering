...
class GraphicsCanvas(QGraphicsView):
    ...
    def _draw_fixed_axes(self, painter: QPainter):
        painter.save()
        painter.resetTransform()

        margin = 45
        ox = margin
        oy = self.viewport().height() - margin
        length = 70
        arrow  = 16
        aw     = 9

        pen = QPen(QColor(100, 220, 255, 240), 2.8)
        pen.setCosmetic(True)
        painter.setPen(pen)
        painter.setBrush(QColor(80, 200, 255, 220))

        # Axe X
        painter.drawLine(ox, oy, ox + length, oy)
        path = QPainterPath()
        path.moveTo(ox + length, oy)
        path.lineTo(ox + length - arrow, oy - aw / 2)
        path.lineTo(ox + length - arrow * 0.4, oy)
        path.lineTo(ox + length - arrow, oy + aw / 2)
        path.closeSubpath()
        painter.drawPath(path)

        # Axe Y
        painter.drawLine(ox, oy, ox, oy - length)
        path = QPainterPath()
        path.moveTo(ox, oy - length)
        path.lineTo(ox - aw / 2, oy - length + arrow)
        path.lineTo(ox, oy - length + arrow * 0.4)
        path.lineTo(ox + aw / 2, oy - length + arrow)
        path.closeSubpath()
        painter.drawPath(path)
        ...
        painter.drawText(ox + length - 16, oy + 16, "X")
        painter.drawText(ox - 26, oy - length + 18, "Y")

        # Point origine lumineux
        grad = QRadialGradient(QPointF(ox, oy), 14)
        grad.setColorAt(0, QColor(255, 255, 255, 200))
        grad.setColorAt(0.6, QColor(100, 220, 255, 180))
        grad.setColorAt(1, QColor(100, 220, 255, 0))
        painter.setBrush(grad)
        painter.setPen(Qt.NoPen)
        painter.drawEllipse(QPointF(ox, oy), 14, 14)
