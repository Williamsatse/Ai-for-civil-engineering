...
from calc_settings_dialog import CalcSettingsDialog
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.settings = {
            ...
            "invert_bmd":       False,
            "language":         "fr",
            # ── Hypothèses de ferraillage ──────────────────────────────────
            "calc_cover":       30.0,   # enrobage a_s en mm (GB 50010)
            "calc_steel_grade": "HRB400",
        }
        self.load_settings()
    ...
    def _create_menus(self):
        ...
        calc_settings_act = calc_menu.addAction(tr("menu_calc_settings"))
        calc_settings_act.triggered.connect(self.open_calc_settings)
    ...
    def open_calc_settings(self):
        """Ouvre le dialogue des hypothèses de ferraillage (enrobage + grade acier)."""
        dlg = CalcSettingsDialog(self)
        dlg.exec()
    ...
    def Area_rebars(self):
        ...
        # ── Hypothèses de ferraillage (depuis Calculs → Hypothèses de ferraillage) ──
        cover       = float(self.settings.get("calc_cover", 30.0))
        steel_grade = self.settings.get("calc_steel_grade", "HRB400")
        a_prime     = max(10.0, cover - 10.0)   # enrobage compression = enrobage - 10 mm

        rc = RectangularSection(
            b=section_obj.b, h=section_obj.h,
            concrete_grade=current_grade, steel_grade=steel_grade,
        ...
        txt = (
            ...
            f"{current_grade} (fc={fc_val} N/mm²)  |  {steel_grade}  |  h₀={h0_val:.1f} mm"
            f"</span>"
            f"<span style='color:#666; font-size:11px;'> — enrobage a<sub>s</sub> = {cover:.0f} mm</span>"
            f"<br>"
